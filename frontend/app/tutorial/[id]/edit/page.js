...
import { BookOpen, Plus, X, Loader2, ArrowLeft, GripVertical, ListOrdered, Hash, Image as ImageIcon, Save, Youtube, Play } from 'lucide-react';
...
export default function EditTutorialPage() {
    ...
    const fetchData = async () => {
        try {
            ...
            const [categoriesRes, tutorialRes] = await Promise.all([
                categoryAPI.list(),
                tutorialAPI.getById(tutorialId)
            ]);

            setCategories(categoriesRes.data.data || []);

            const tutorial = tutorialRes.data.data;

            // Check ownership
            const creatorId = typeof tutorial.creatorId === 'object' ? tutorial.creatorId._id : tutorial.creatorId;
            ...
            reset({
                ...
                categoryId: typeof tutorial.categoryId === 'object' ? tutorial.categoryId._id : tutorial.categoryId,
                type: tutorial.type || 'course',
                difficulty: tutorial.difficulty || 'medium',
                videoUrl: tutorial.videoUrl || '',
                lessons: tutorial.lessons || [{ title: '', topics: [{ title: '', videoUrl: '' }] }]
            });
    ...
    const onSubmit = async (data) => {
        ...
        setSaving(true);
        try {
            await tutorialAPI.update(tutorialId, {
                ...data,
                tags,
                thumbnailUrl
            });
            ...
            <main className="flex-1">
                <div className="container px-4 py-12">
                    <div className="max-w-4xl mx-auto">
                        <Link href={`/tutorial/${tutorialId}`} className="text-muted-foreground hover:text-foreground text-sm flex items-center mb-8 transition-colors">
                            <ArrowLeft className="h-4 w-4 mr-1" />
                            Back to Course
                        </Link>

                        <div className="mb-10">
                            <h1 className="text-4xl font-bold tracking-tight mb-2">Edit Course</h1>
                            <p className="text-muted-foreground text-lg">Update your structured learning journey.</p>
                        </div>
                        ...
                        <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
                            ...
                            <div className="pt-6">
                                <Button type="submit" className="w-full py-8 text-xl font-bold shadow-lg shadow-primary/20" disabled={saving}>
                                    {saving ? (
